...
$auc_codlast=Sanitize::filter($_GET['idlast']);
...
//usuário banido
$query17b = mysql_query("SELECT * FROM ban_user_commu  where IdCommunity='$auc_codcommet' and UserId='".USERID_ID."' limit 1 ")or die(mysql_error());;
...
// mysql_query("UPDATE ask_forum SET status_ask='' where  idforum='$auc_cod' and status_ask='1' ");
 ...
 $query55 = mysql_query("SELECT * FROM ask_forum where  idforum='$auc_cod' and abuse<>'abuse' and  timestamp='$auc_codlast' order by idorder ASC limit $inicio_aux3,$qtd ")or die(mysql_error());
   $rcont334 = mysql_num_rows($query55);
       if ($rcont334!='0'){
       
                $mensagem = mysql_fetch_object($query55);
...
<?php }else{?>
<div     class="ui_mw_postoosm user_foto_borda_superior sombra_padrao_bottom"
    style="position:relative; width: 500px; height: 300px;display:;margin-bottom:20px; background-color:#FFFFFF ;">
    <div style="margin: 20px; width: 100%; height: 100%; position: relative">
        <div style="position: absolute; width: 100%; height: 100px; background-color: #FFFFDD; border: 1px dotted #FFFF99; top: 100px;">
            <span style="font-family: arial, Helvetica, sans-serif; font-size: 20px; font-weight: normal; font-style: normal; font-variant: normal; color: #FF0000; position: absolute; top: 27px; left: 83px;">Você não tem acesso a este tópico!</span></div></div>
</div>
<?php };?>
<?php }else{
exit;
