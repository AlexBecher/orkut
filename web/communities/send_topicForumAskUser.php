...
$fotothumb=Sanitize::filter($_POST['foto39ur3r6i2ryy4']);
 ...
 //insert msgpost
if ($foto_img_salt!='' or  $_POST['form_data']!=''){
...
$link_video_001='';
   ...
   // match www.something
$UserMsg=sanitize($texto);
//verifica postagens repetidas
 include('app/crip/cod_baseVal.php');
$aux_post_session =add_cod_transfer("$IdOrderEventos.$UserId_Cokie.$UserName_cokie.$fotothumb.$video.$path.$foto_img_salt.$UserMsg"); 
...
$query55d3 = mysql_query("SELECT UserId,COUNT(UserId) AS quant FROM ask_forum where  idforum='$IdOrderEventos' and ($aux_0304 - timestamp < '86400') GROUP BY UserId ORDER BY quant DESC limit 500 ")or die(mysql_error());
    $rcont37 = mysql_num_rows($query55d3);
if ($rcont37!='0'){
while ($mensagem = mysql_fetch_object($query55d3)){
$query17 = mysql_query("SELECT UserId FROM list_community_user  where IdCommunity='$mensagem33jweje->IdCommunity' and UserId='".$mensagem->UserId."' limit 1 ")or die(mysql_error());;
...
$querycod = mysql_query("SELECT UserIdPro FROM community_users  where UserIdPro='".$mensagem->UserId."' and UserIdPro<>'".$UserId_Cokie."'  limit 1 ")or die(mysql_error());;
$rcont37b8938ye38r = mysql_num_rows($querycod);

if ($mensagem->UserId!=$UserId_Cokie && $rcont37!='0' && $rcont37b=='0' or $rcont37b8938ye38r!='0'){
