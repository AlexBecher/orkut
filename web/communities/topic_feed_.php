...
$queryMy = mysql_query("SELECT community_users.IdCommunity,community_users.CommunityImg,community_users.CommunityName  FROM community_users WHERE NOT EXISTS(SELECT IdCommunity FROM list_community_user WHERE (community_users.UserIdPro='".USERID_ID."' ) or (community_users.IdCommunity = list_community_user.IdCommunity and list_community_user.UserId='".USERID_ID."'))and community_users.CommunityPrivacy='2'  ORDER BY community_users.timestamp2 DESC limit 4")or die(mysql_error());
